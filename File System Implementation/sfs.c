...
void display(char *fname) {
	...
	if (found) { // display file contents to standard output enter your code below
		int f_inode = e_inode;
		itype = _inode_table[f_inode].TT[0];
		blocks[0] = stoi(_inode_table[f_inode].XX,2);
		blocks[1] = stoi(_inode_table[f_inode].YY,2);
		blocks[2] = stoi(_inode_table[f_inode].ZZ,2);
		...
		for (i=0; i<3; i++) {
			...
			readSFS(blocks[i],buff);	// lets read a directory entry; notice the cast
...
void create(char *fname){
	...
	for (i=0; i<3; i++) {
		...
		for (j=0; j<4; j++) {
			...
			int e_inode = stoi(_directory_entries[j].MMM,3);	// this is the inode that has more info about this entry
	...
	if (empty_dentry==-1 && empty_dblock==-1) {
		...
		int MAX=3072, i=0;
		char buffer[MAX];
		printf("Enter File Data(Max. 3072 characters): (Press Esc To Quit)\n");
		while(i<MAX){
			char tmp;
			tmp = getchar();
		...
		for(int i=0;i<3;i++){
			...
			if(strlen(tmp)!=0){
				if ((blk[i] = getBlock())==-1) {  // first get a new block using the block bitmap
					printf("Error: Disk is full.\n");
					return;
				}
				writeSFS(blk[i], tmp);
		...
		// printf("%d %d %d",blk[0],blk[1],blk[2]);
		if(blk[0]!=0)
			itos(_inode_table[empty_ientry].XX,blk[0],2);
		else
			strncpy(_inode_table[empty_ientry].XX,"00",2);
		if(blk[1]!=0)
			itos(_inode_table[empty_ientry].YY,blk[1],2);
		else
			strncpy(_inode_table[empty_ientry].YY,"00",2);
		if(blk[2]!=0)
			itos(_inode_table[empty_ientry].ZZ,blk[2],2);
		else
			strncpy(_inode_table[empty_ientry].ZZ,"00",2);
...
void rm(char *fname){
	...
	if(found){
		...
		type = _inode_table[f_inode].TT[0];
		blks[0] = stoi(_inode_table[f_inode].XX,2);
		blks[1] = stoi(_inode_table[f_inode].YY,2);
		blks[2] = stoi(_inode_table[f_inode].ZZ,2);
		...
		if(type=='F'){
			...
			readSFS(blocks[block],(char *)_directory_entries);
			_directory_entries[entry].F='0';
			writeSFS(blocks[block],(char *)_directory_entries);

			int flag=1;
			for(int i=0;i<4;i++){
				if(_directory_entries[i].F!='0')
					flag=0;
			}
			if(flag)
		...
		if(type=='D'){
			int cd=CD_INODE_ENTRY;
			CD_INODE_ENTRY=f_inode;
			for (i=0; i<3; i++) {
				if (blks[i]==0) continue;	// 0 means pointing at nothing
				
				readSFS(blks[i],(char *)_directory_entries);	// lets read a directory entry; notice the cast
				
				// so, we got four possible directory entries now
				for (j=0; j<4; j++) {
					if (_directory_entries[j].F=='0') continue;	// means unused entry
					
					char *name=_directory_entries[j].fname;
					// int cd = CD_INODE_ENTRY;
					// CD_INODE_ENTRY=f_inode;
					rm(name);
					// CD_INODE_ENTRY=cd;
				...
				writeSFS(blks[i],(char *)_directory_entries);
				int flag=1;
				for(int i=0;i<4;i++){
					if(_directory_entries[i].F!='0')
						flag=0;
